...
25th August 2026
**What:** Added `admin-management.html`, restricted to `super_admin` (non-super-admins redirected away), combining: a system statistics stat-card grid (member/session/user counts, role breakdown, audit log count) and Recent Activity list wired to the Entry 19 endpoint, plus an "Add New User" form and a table of all users with edit/delete actions wired to the Entry 18 endpoints. Added a "User Management" link to the shared nav, shown only to super admins.
**Why:** The API endpoints from Entries 18–19 needed an actual interface — without this, adding, removing, or reviewing users meant editing JSON by hand, defeating the purpose of having roles at all. Built as one page/commit since both pieces ship together as a single admin view, rather than as a half-finished page followed by a second commit adding what was obviously always meant to be there. Extended the existing `initPage()` to also toggle the new nav link, rather than adding a separate `initNav()` function that would re-fetch `/api/check-auth` and re-check the role a second time on every page load — deliberately avoiding a duplication pattern noticed in the original repo.
**Next:** Build the actual SVdP-required roll & return export format (closes the still-open 17 March Issue) — this has been deferred across several stages now and is the next priority.